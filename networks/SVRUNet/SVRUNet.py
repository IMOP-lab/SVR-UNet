...
# timm is optional (not used directly in this file)
try:
...
_COORDS_2D_CACHE_MAXSIZE = 32
_COORDS_2D_CACHE = OrderedDict()
_COORDS_2D_CACHE_LOCK = Lock()


def _device_cache_id(device: torch.device):
...
def ihaar_2d(LL: torch.Tensor, LH: torch.Tensor, HL: torch.Tensor, HH: torch.Tensor):
    ...
    L = torch.stack((W_even_L, W_odd_L), dim=-1).reshape(N, C, h, 2 * w)
    Hc = torch.stack((W_even_H, W_odd_H), dim=-1).reshape(N, C, h, 2 * w)

    x_even = L + Hc
    x_odd = L - Hc
    return torch.stack((x_even, x_odd), dim=3).reshape(N, C, 2 * h, 2 * w)


def coords_2d(h: int, w: int, device, dtype):
    device = torch.device(device)
    key = (h, w, device.type, _device_cache_id(device), dtype, torch.is_inference_mode_enabled())
    with _COORDS_2D_CACHE_LOCK:
        coords = _COORDS_2D_CACHE.get(key)
        if coords is not None:
            _COORDS_2D_CACHE.move_to_end(key)
            return coords

    yy = torch.linspace(-1, 1, h, device=device, dtype=dtype)
    xx = torch.linspace(-1, 1, w, device=device, dtype=dtype)
    coords = torch.empty((1, 2, h, w), device=device, dtype=dtype)
    coords[:, 0].copy_(yy.view(1, h, 1).expand(1, h, w))
    coords[:, 1].copy_(xx.view(1, 1, w).expand(1, h, w))

    with _COORDS_2D_CACHE_LOCK:
        cached = _COORDS_2D_CACHE.get(key)
        if cached is not None:
            _COORDS_2D_CACHE.move_to_end(key)
            return cached
        _COORDS_2D_CACHE[key] = coords
        if len(_COORDS_2D_CACHE) > _COORDS_2D_CACHE_MAXSIZE:
            _COORDS_2D_CACHE.popitem(last=False)
    return coords  # (1,2,h,w)


# ============================================================
#            wavelet subband gating (Down W-view)
# ============================================================
...
# ============================================================
#           View attention with axis-gradient prior (Down)
# ============================================================
...
# ============================================================
#        Optimized TriPlaneFreqAttDown (balanced & faster)
# ============================================================
class TriPlaneFreqAttDown(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # axis refine
        if not torch.is_grad_enabled():
            axis = self.axis_dw_d(x_va)
            axis.add_(self.axis_dw_h(x_va))
            axis.add_(self.axis_dw_w(x_va))
        else:
...
# ============================================================
#        PlaneWaveletLifting2x + Up module
# ============================================================
...
class PlaneWaveletLifting2x(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if not torch.is_grad_enabled():
            y_interp.mul_(1.0 - conf_up)
            y_interp.addcmul_(y_wave, conf_up)
            y = y_interp
        else:
...
class AnisoTriPlaneWaveletUp(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # axis refine
        if not torch.is_grad_enabled():
            axis = self.axis_dw_d(y_tri)
            axis.add_(self.axis_dw_h(y_tri))
            axis.add_(self.axis_dw_w(y_tri))
        else:
...
class TSPD(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = torch.addcmul(x_base, x_tri, gate)
        out = self.eca(out)
        return out
...
class TSPP(nn.Module):
    ...
    def forward(self, x: torch.Tensor, x_e: Optional[torch.Tensor]) -> torch.Tensor:
        ...
        out = torch.addcmul(y_base, y_tri, g)
        return out
