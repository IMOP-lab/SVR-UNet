...
# 检查模型权重文件是否存在
if os.path.exists(os.path.join(args.output, 'best_metric_model.pth')):
    print("######### Pretrained best metric model loaded! ##########")
    print(os.path.join(args.output, 'best_metric_model.pth'))
...
## Learning rate adjustment strategy
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'max', factor=0.9, patience=4000)    # maximizing dice, and the learning rate is reduced by 10% if there is no improvement within 5 cycles.
